    ...
    [manager POST:urlString parameters:params success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            if ([responseObject[@"statusCode"] isEqualToString:@"200"]) {
                ...
                NSError *error = [NSError errorWithDomain:errorDomainValue code:[responseObject[@"statusCode"]intValue] userInfo:@{NSURLErrorFailingURLStringErrorKey:[XRQController returnStr:responseObject[@"msg"]]}];
...
//    [HUD loading];
    ...
    [manager POST:urlString parameters:params success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        if ([responseObject[@"result"] intValue]) {
            ...
            NSDictionary *dic = [XRQController dictionaryWithJsonString:errStr];
