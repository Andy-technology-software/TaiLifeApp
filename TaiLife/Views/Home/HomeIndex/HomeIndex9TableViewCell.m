...
@interface HomeIndex9TableViewCell()
...
@end
@implementation HomeIndex9TableViewCell
...
- (void)makeUI{
    self.title0 = [XRQController createLabelWithFrame:self.contentView.frame Font:14 Text:@"同城热卖"];
...
- (void)configCellWithModel:(HomeIndex9Model *)model {
    float viewWidth = ([XRQController getScreenWidth] - 30) / 2;
    ...
    self.title2 = [XRQController createLabelWithFrame:self.contentView.frame Font:12 Text:@"了解更多产品"];
    ...
    for (int i = 0; i < model._imgArr.count; i++) {
        UIView* _BV = [XRQController viewWithFrame:CGRectMake((viewWidth + 10) * (i % 2) + 10, (viewWidth * 359 / 294 + 10) * ceil(i / 2), viewWidth, viewWidth * 359 / 294)];
        ...
        UIImageView* _imageV = [XRQController createImageViewWithFrame:CGRectMake(0, 0, viewWidth, viewWidth * 272 / 294) ImageName:nil];
        [_imageV sd_setImageWithURL:[NSURL URLWithString:model._imgArr[i]] placeholderImage:[UIImage imageNamed:@""]];
        [_BV addSubview:_imageV];
        
        UILabel* titleL = [XRQController createLabelWithFrame:CGRectMake(2, CGRectGetMaxY(_imageV.frame) + 8, _imageV.frame.size.width - 4, 15) Font:10 Text:model._titleArr[i]];
        ...
        UIButton* clickB = [XRQController createButtonWithFrame:CGRectMake(_imageV.frame.origin.x, 0, _imageV.frame.size.width , viewWidth * 359 / 294) ImageName:nil Target:self Action:@selector(clickBClick:) Title:nil];
        clickB.tag = 100 + i;
...
- (void)clickBClick:(UIButton*)btn{
    [self.HomeIndex9TableViewCellDelegate sendBack9Item:btn.tag - 100];
    
}

- (void)moreBtnClick {
    [self.HomeIndex9TableViewCellDelegate s9MoreClick];
